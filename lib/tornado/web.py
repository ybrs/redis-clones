...
class RequestHandler(object):
    ...
    def __init__(self, application, request, transforms=None, handling_method=None):
    ...
    def get_param(self, name, default=None, strip=True):
    ...
    def preexecute(self, method, action ):
      pass

    def postexecute(self, method, action):
      pass
    ...
    def _execute(self, transforms, *args, **kwargs):
        ...
        try:
            ...
            self.filterchain.request = self
            self.filterchain.response = self._write_buffer
            self.filterchain.add(app_cfg.filters)
            self.filterchain.execute()

#            # TODO: these filters must be in application
#            for i in app_cfg.filters:
#              filter = i(self, self._write_buffer)
#              filter.execute()

#            self.prepare()
#            if not self._finished:
#                if not self.handling_method == Application._default_handler_method:
#                  action_name = '%s_%s' % (self.request.method.upper(), self.handling_method)
#                  try:
#                    self.preexecute(self.request.method.upper(), self.handling_method)
#                    getattr(self, action_name)(*args, **kwargs)
#                  except Exception, e:
#                    self._handle_request_exception(e)
#                else:
#                  self.preexecute(self.request.method.upper(), None)
#                  getattr(self, self.request.method.lower())(*args, **kwargs)
#
#                self.postexecute(self.request.method.upper(), self.handling_method)
#
...
class Application(object):
    ...
    def add_handlers(self, host_pattern, host_handlers):
        ...
        for handler_tuple in host_handlers:
            ...
            if (type(handler) == type(tuple())):
              pass
            else:
              handler = (handler, Application._default_handler_method)
    ...
    def __call__(self, request):
        ...
        if not handlers: 
            ...
            for pattern, handler_tuple, kwargs in handlers:
                match = pattern.match(request.path)
                if match:
                    handler_class = handler_tuple[0]
                    # XXX
                    handler = handler_class(self, request, **kwargs)
                    handler.handling_method = handler_tuple[1]
